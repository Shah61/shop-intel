...
const EngagementRateSection: React.FC<{
  ...
  const chartData = useMemo(() => {
    ...
    const main = data.data.engagement_rate_comparison.slice(0, 10).map(c => ({
  ...
  return (
    <Panel>
      <PanelHeader title="Engagement Rate Comparison" subtitle="Top 10 accounts by engagement + Shop-Intel" icon={<BarChart3 style={{ width: 14, height: 14 }} />} iconColor="#8b5cf6" />
      {isLoading ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 10 }}>
            {[{ l: 'Creator', c: '#10b981' }, { l: 'Competitor', c: '#f59e0b' }, { l: 'Shop-Intel', c: '#8b5cf6' }].map((x, i) => (
...
const EngagementGrowthSection: React.FC<{
  ...
  const chartData = useMemo(() => {
    ...
    return data.data.engagement_growth_trend.map(dp => {
      ...
      if (dataType === 'avg') {
        return { date: format(date, 'MMM dd'), competitor: dp.avg.competitors_avg, 'Shop-Intel': dp.avg['Shop-Intel_avg'], creator: dp.avg.creator_avg };
      }
      return { date: format(date, 'MMM dd'), competitor: dp.highest.competitors_highest, 'Shop-Intel': dp.highest['Shop-Intel_highest'], creator: dp.highest.creator_highest };
    });
  }, [data, dataType]);
        ...
        <>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={chartData} margin={{ top: 0, right: 0, left: -22, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="Shop-Intel" name="Shop-Intel" stroke="#f59e0b" strokeWidth={2} dot={{ r: 2, fill: '#f59e0b', strokeWidth: 0 }} />
              <Line type="monotone" dataKey="creator" name="Creators" stroke="#06b6d4" strokeWidth={2} dot={{ r: 2, fill: '#06b6d4', strokeWidth: 0 }} />
            </ComposedChart>
          </ResponsiveContainer>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 10 }}>
            {[{ l: 'Competitors', c: '#8b5cf6' }, { l: 'Shop-Intel', c: '#f59e0b' }, { l: 'Creators', c: '#06b6d4' }].map((x, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10 }}>
...
const TopPerformersSection: React.FC<{
  ...
  const title = source === 'CREATOR' ? 'Top Performing Creators' : source === 'COMPETITOR' ? 'Top Performing Competitors' : source === 'Shop-Intel' ? 'Shop-Intel Performance' : 'Top Performing Channels';
      ...
      <div style={{ padding: '15px 20px', borderBottom: '1px solid rgba(255,255,255,.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '-0.2px' }}>{title}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,.38)', marginTop: 1 }}>{totalCompetitors} channels · Click rows for details</div>
            </div>
          </div>
        </div>
                  ...
                  <tr key={ch.id} onClick={() => onSelectCompetitor(ch.id, ch.name)} style={{ borderBottom: '1px solid rgba(255,255,255,.06)', cursor: 'pointer', transition: 'background .15s' }}
                    ...
                    <td style={{ padding: '10px 15px 10px 11px', minWidth: 130 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 800, color: engC, minWidth: 36, textAlign: 'right' }}>{eng.toFixed(2)}%</span>
                      </div>
                    </td>
