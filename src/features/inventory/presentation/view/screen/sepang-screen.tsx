...
import { useGetSkuDetailsSepang } from "../../tanstack/inventory-tanstack";
import { WarehouseSkuOverview } from "../components/warehouse-sku-overview";

export const SepangScreen = () => {
    const [searchTerm, setSearchTerm] = useState("");
    const [stockFilter, setStockFilter] = useState("all");
    const { skuDetailsSepang, isLoading, error } = useGetSkuDetailsSepang();

    // Filter SKUs based on search term and stock status
    const filteredSkus = useMemo(() => {
        if (!skuDetailsSepang?.data?.skus) return [];
        
        let filtered = skuDetailsSepang.data.skus;
        ...
        return filtered;
    }, [skuDetailsSepang, searchTerm, stockFilter]);
        ...
        return (
            <div className="flex items-center justify-center py-20 w-full">
                <div className="text-center">
                    <Warehouse className="h-16 w-16 text-red-500/30 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold mb-2 text-red-600">Error Loading Sepang Data</h3>
                    <p className="text-muted-foreground">Failed to load warehouse inventory. Please try again.</p>
                </div>
            </div>
        );
    }
            ...
            <Card>
                <CardContent className="pt-6 space-y-4">
                    ...
                    <div className="text-sm text-muted-foreground">
                        Showing {filteredSkus.length} of {skuDetailsSepang?.data?.skus.length || 0} items
