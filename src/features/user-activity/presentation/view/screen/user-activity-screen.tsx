...
const DUMMY_EVENTS: EventEntity[] = [
    ...
    { id: "evt-007", name: "Category Created", description: "New category 'Accessories' added under 'Fashion'", type: "CATEGORY", status: "completed", created_at: "2025-03-28T11:00:00Z", updated_at: "2025-03-28T11:00:00Z", user: DUMMY_USERS[5] },
    ...
    return (
        <div className="flex flex-col gap-4 w-full" style={{ fontFamily: "'Outfit', sans-serif" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, width: "100%" }} className="overview-platform-grid !grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-4">
                <OverviewDataCard customTitle="Total Events" customIcon={<Activity size={22} strokeWidth={2} />} metricSubtitle="All tracked events" primaryValueDisplay={String(meta.total)} expandLabel1="Today" expandValue1Display={String(meta.todayCount)} expandLabel2="Unique Users" expandValue2Display={String(meta.uniqueUsers)} isLoading={isLoading} expanded={cardsExpanded} onExpandToggle={() => setCardsExpanded((e) => !e)} />
                <OverviewDataCard customTitle="Completed" customIcon={<CheckCircle2 size={22} strokeWidth={2} />} metricSubtitle="Successfully completed" primaryValueDisplay={String(meta.completed)} expandLabel1="Success Rate" expandValue1Display={meta.total > 0 ? `${((meta.completed / meta.total) * 100).toFixed(0)}%` : "—"} expandLabel2="Pending" expandValue2Display={String(meta.pending)} isLoading={isLoading} expanded={cardsExpanded} onExpandToggle={() => setCardsExpanded((e) => !e)} />
                <OverviewDataCard customTitle="Failed" customIcon={<AlertTriangle size={22} strokeWidth={2} />} metricSubtitle="Errors & failures" primaryValueDisplay={String(meta.failed)} expandLabel1="Failure Rate" expandValue1Display={meta.total > 0 ? `${((meta.failed / meta.total) * 100).toFixed(0)}%` : "—"} expandLabel2="In Progress" expandValue2Display={String(events.filter((e) => e.status === "in_progress").length)} isLoading={isLoading} expanded={cardsExpanded} onExpandToggle={() => setCardsExpanded((e) => !e)} />
                <OverviewDataCard customTitle="Active Users" customIcon={<Users size={22} strokeWidth={2} />} metricSubtitle="Unique users with activity" primaryValueDisplay={String(meta.uniqueUsers)} expandLabel1="Top Type" expandValue1Display={meta.topType ? meta.topType[0] : "—"} expandLabel2="Count" expandValue2Display={meta.topType ? String(meta.topType[1]) : "—"} isLoading={isLoading} expanded={cardsExpanded} onExpandToggle={() => setCardsExpanded((e) => !e)} />
            </div>
